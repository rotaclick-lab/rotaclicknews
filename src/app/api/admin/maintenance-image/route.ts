...
/** POST — faz upload da imagem de manutenção e salva a URL em platform_settings */
export async function POST(req: NextRequest) {
  ...
  // Salva URL em platform_settings
  const { data: { user } } = await (await createClient()).auth.getUser()
  await admin.from('platform_settings').upsert(
    { key: 'maintenance_image_url', value: publicUrl, updated_by: user?.id, updated_at: new Date().toISOString() },
    { onConflict: 'key' }
  )
...
/** DELETE — remove imagem de manutenção */
export async function DELETE() {
  ...
  await admin.from('platform_settings').upsert(
    { key: 'maintenance_image_url', value: '', updated_at: new Date().toISOString() },
    { onConflict: 'key' }
  )
