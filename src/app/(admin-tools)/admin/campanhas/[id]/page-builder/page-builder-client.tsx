  ...
  if (block.type === 'image') {
    return (
      <div className="flex flex-col items-center gap-2 py-4">
        {c.src ? (
          <div className={`relative w-full aspect-video ${c.rounded === 'true' ? 'rounded-xl overflow-hidden' : ''}`} style={{ width: `${c.width ?? 100}%` }}>
  ...
  if (block.type === 'cta') {
    return (
      <div className="py-10 px-6 rounded-xl text-center" style={{ backgroundColor: String(c.bg_color ?? '#f8fafc') }}>
        ...
        <span className="inline-block px-6 py-3 rounded-lg font-semibold text-white" style={{ backgroundColor: String(c.button_color ?? '#2BBCB3') }}>
...
export function PageBuilderClient({ campaign }: { campaign: Campaign }) {
  ...
  const addBlock = useCallback((type: PageBlock['type']) => {
    ...
    const newBlock: PageBlock = { id: `block-${Date.now()}`, type, content: { ...def.defaultContent } }
