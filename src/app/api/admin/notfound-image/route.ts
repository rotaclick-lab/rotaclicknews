...
/** POST — faz upload da imagem 404 e salva a URL em platform_settings */
export async function POST(req: NextRequest) {
  ...
  const fileName = `notfound-${Date.now()}.${ext}`
  ...
  await admin.from('platform_settings').upsert(
    { key: 'notfound_image_url', value: publicUrl, updated_by: user?.id, updated_at: new Date().toISOString() },
    { onConflict: 'key' }
  )
...
export async function DELETE() {
  ...
  await admin.from('platform_settings').upsert(
    { key: 'notfound_image_url', value: '', updated_at: new Date().toISOString() },
    { onConflict: 'key' }
  )
