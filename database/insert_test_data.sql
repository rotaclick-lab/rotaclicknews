    -- ============================================
    -- ROTACLICK - INSERIR DADOS DE TESTE
    ...
    -- ============================================
    -- IMPORTANTE: 
    -- Este script usa auth.uid() para pegar automaticamente
    -- o company_id do usuário logado. Execute no Supabase
    -- SQL Editor estando logado!
    -- ============================================
    ...
    (SELECT company_id FROM profiles WHERE id = auth.uid() LIMIT 1),
    (SELECT id FROM customers WHERE cpf_cnpj = '12.345.678/0001-90' LIMIT 1),
    ...
    SELECT COUNT(*) INTO freights_count FROM freights 
        WHERE company_id = (SELECT company_id FROM profiles WHERE id = auth.uid() LIMIT 1);
    
    SELECT COUNT(*) INTO transactions_count FROM transactions 
        WHERE company_id = (SELECT company_id FROM profiles WHERE id = auth.uid() LIMIT 1);
    
    SELECT COUNT(*) INTO routes_count FROM marketplace_routes 
        WHERE company_id = (SELECT company_id FROM profiles WHERE id = auth.uid() LIMIT 1);
